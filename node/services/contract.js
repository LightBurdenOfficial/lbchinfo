...
const totalSupplyABI = Solidity.lpp20ABIs.find(abi => abi.name === 'totalSupply')
const balanceOfABI = Solidity.lpp20ABIs.find(abi => abi.name === 'balanceOf')
const ownerOfABI = Solidity.lpp721ABIs.find(abi => abi.name === 'ownerOf')
...
class ContractService extends Service {
  ...
  async start() {
    ...
    this.#LPP20 = this.node.getModel('lpp20')
    this.#LPP20Balance = this.node.getModel('lpp20_balance')
    this.#LPP721 = this.node.getModel('lpp721')
    this.#LPP721Token = this.node.getModel('lpp721_token')
    ...
    for (let transaction of block.transactions) {
      for (let i = 0; i < transaction.outputs.length; ++i) {
        ...
        if (output.scriptPubKey.type === OutputScript.EVM_CONTRACT_CREATE) {
          ...
          if (contract && contract.type === 'lpp20') {
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      await this.#db.query(sql`
        DELETE contract, tag, lpp20, lpp20_balance, lpp721, lpp721_token
        FROM contract
        LEFT JOIN contract_tag tag ON tag.contract_address = contract.address
        LEFT JOIN lpp20 ON lpp20.contract_address = contract.address
        LEFT JOIN lpp20_balance ON lpp20_balance.contract_address = contract.address
        LEFT JOIN lpp721 ON lpp721.contract_address = contract.address
        LEFT JOIN lpp721_token ON lpp721_token.contract_address = contract.address
  ...
  async _createContract(address, vm) {
    ...
    if (isLPP721(code)) {
      let [nameResult, symbolResult, totalSupplyResult] = await this._batchCallMethods([
        {address, abi: Solidity.lpp721ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.lpp721ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.lpp721ABIs.find(abi => abi.name === 'totalSupply')}
      ])
      try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'lpp721'})
        await this.#LPP721.create({
    ...
    } else if (isLPP20(code)) {
      let [
        nameResult, symbolResult, decimalsResult, totalSupplyResult, versionResult
      ] = await this._batchCallMethods([
        {address, abi: Solidity.lpp20ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.lpp20ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.lpp20ABIs.find(abi => abi.name === 'decimals')},
        {address, abi: Solidity.lpp20ABIs.find(abi => abi.name === 'totalSupply')},
        {address, abi: Solidity.lpp20ABIs.find(abi => abi.name === 'version')}
      ])
      try {
        let version
        try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'lpp20'})
        await this.#LPP20.create({
          contractAddress: address,
          name,
          symbol,
          decimals,
  ...
  async _processReceipts(block) {
    ...
    for (let addressString of totalSupplyChanges) {
      ...
      if (contract) {
        ...
        if (contract.type === 'lpp20') {
          await this.#LPP20.update({totalSupply}, {where: {contractAddress: address}})
        } else {
          await this.#LPP721.update({totalSupply}, {where: {contractAddress: address}})
        }
      }
    }
  }
  ...
  async _updateBalances(balanceChanges) {
    ...
    if (operations.length) {
      await this.#LPP20Balance.bulkCreate(operations, {updateOnDuplicate: ['balance'], validate: false})
    }
  }
  ...
  async _updateTokenHolders(transfers) {
    ...
    await this.#LPP721Token.bulkCreate(operations, {updateOnDuplicate: ['holder'], validate: false})
  }
}

function isLPP20(code) {
  return code.includes(balanceOfABI.id)
    && code.includes(transferABI.id)
    && code.includes(TransferABI.id)
}

function isLPP721(code) {
  return code.includes(balanceOfABI.id)
